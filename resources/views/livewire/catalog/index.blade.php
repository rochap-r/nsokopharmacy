<div class="max-w-7xl mx-auto py-8 sm:px-6 lg:px-8" x-data="{ showDeleteModal: false, productIdToDelete: null }">
    <header class="mb-8">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white mb-4 md:mb-0">
                {{ $title ?? 'Catalogue des Produits' }}</h1>
            <div class="flex flex-wrap gap-3">
                @can('catalog.import')
                    <a href="{{ tenant_route('tenant.catalog.import-export') }}"
                ...
                @endcan
                @can('catalog.view')
                    <a href="{{ tenant_route('tenant.catalog.aisles.index') }}"
                ...
                @endcan
                @can('catalog.create')
                    <a href="{{ tenant_route('tenant.catalog.products.create') }}"
                        class="inline-flex items-center px-4 py-2.5 text-sm font-medium rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 dark:bg-green-700 dark:hover:bg-green-600 transition-colors duration-200"
                        wire:navigate>
                        {{-- modifie l'icone mets un + --}}
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @endcan
                @can('catalog.view')
                    <a href="{{ tenant_route('tenant.catalog.categories.index') }}"
    ...
    <div
        class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl rounded-xl dark:border dark:border-gray-700 transition-all duration-200">
        <!-- Filtres de recherche -->
        <div class="p-6 bg-gray-50 dark:bg-gray-700/50 rounded-xl border-b border-gray-200 dark:border-gray-600/50">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div class="space-y-2">
                    <label for="search" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Rechercher
                        un produit</label>
                    <div class="relative">
                ...
                <div class="space-y-2">
                    <label for="active"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Statut</label>
                        ...
                        @forelse($products as $product)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    {{ $product->dosage }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    {{ $product->forme_galenique }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    @if ($product->category)
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                            style="background-color: {{ $product->category->color_code ?? '#e5e7eb' }}; color: {{ $product->category->color_code ? '#ffffff' : '#374151' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    @if ($product->active)
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-100">Actif</span>
                                    @else
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 dark:bg-red-800 text-red-800 dark:text-red-100">Inactif</span>
                                    @endif
                                </td>
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center justify-end space-x-2">
                                    <div class="flex space-x-2">
                                        @can('catalog.view')
                                            <a href="{{ tenant_route('tenant.catalog.products.show', ['id' => $product->id]) }}"
                                        ...
                                        @endcan
                                        @can('catalog.edit')
                                            <a href="{{ tenant_route('tenant.catalog.products.edit', ['id' => $product->id]) }}"
                                        ...
                                        @can('catalog.delete')
                                            <button
                                                @click="showDeleteModal = true; productIdToDelete = {{ $product->id }}"
        ...
        <div x-show="showDeleteModal"
            ...
            <div @click.away="showDeleteModal = false"
                ...
                <div class="text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mt-2">Êtes-vous sûr de vouloir supprimer ce produit?
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <button wire:click="deleteProduct(productIdToDelete)" @click="showDeleteModal = false"
